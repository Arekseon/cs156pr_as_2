...
def HITS(query):
    ...
    while True: # repeat until... convergence
        for p in pages:
            p.authority = sum(x.hub for x in getInlinks(p))  # p.authority ← SUMi Inlinki(p).Hub
            p.hub = sum(x.authority for x in getOutlinks(p)) # p.hub ← SUMi Outlinki(p).Authority
        normalize(pages)
        if convergence():
            break
    return pages
