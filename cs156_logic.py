...
def update_position(x, y, orientation, action):
    if action == 'TurnRight':
        orientation = turn_heading(orientation, +1)
    elif action == 'TurnLeft':
        orientation = turn_heading(orientation, -1)
    elif action == 'Forward':
        x, y = vector_add((x, y), orientation)
...
def vector_add(location, vector):
...
grid_expression = { "S_1,1":Expr("S11"),
                    "~S_1,1":Expr("~S11"),
                    "B_1,1":Expr("B11"),
                    "~B_1,1":Expr("~B11"),
...
# PL-Wumpus-Agent [Fig. 7.19]
class PLWumpusAgent_2(agents.Agent):
    "An agent for the wumpus world that does logical inference. [Fig. 7.19]"""
    def __init__(self):
        import time
        self.KB = PropKB()
        ...
        def program(percept):
            ...
            self.x, self.y, self.orientation = update_position(self.x, self.y, self.orientation, self.action)
            self.KB.tell(grid_expression['%sS_%d,%d' % ('' if stench else '~', self.x, self.y)])
            self.KB.tell(grid_expression['%sB_%d,%d' % ('' if breeze else '~', self.x, self.y)])
            if glitter: action = 'Grab'
            elif plan: action = plan.pop()
            else:
                for [i, j] in visited:
...
# def tell_if_not_there()
# class WumpusWorldAgent_to_delete(Agent):
...
#     e = agents.WumpusEnvironment(width=6, height=6)
#     print("FRESH START")
#     e.add_object(PLWumpusAgent_2(), location = (1,1), direction= (1,0))
#     # e.add_object(agents.Gold(), location = (8,3))
#     # e.add_object(agents.Gold(), location = (3,4))
#     # e.add_object(agents.Gold(), location = (5,7))
    
#     # e.add_object(agents.Wumpus() , location = (3,8))
#     # e.add_object(agents.Pit() , location = (1,3))
#     # e.add_object(agents.Pit() , location = (4,5))
#     e.run(20)