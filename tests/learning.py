...
class DataSet:
    ...
    def check_me(self):
        ...
        """
        if not (len(self.attrnames) == len(self.attrs)):
            print "AssertionError(len(self.attrnames) == len(self.attrs) Failed)"
            sys.exit
        if not (self.target in self.attrs):
            print "AssertionError(self.target in self.attrs Failed)"
            sys.exit
        if not (self.target not in self.input):
            print "AssertionError(self.target not in self.input Failed)"
            sys.exit
        if not set(self.inputs).issubset(set(self.attrs)):
            print "AssertionError(set(self.inputs).issubset(set(self.attrs))  Failed)"
            sys.exit
        """
